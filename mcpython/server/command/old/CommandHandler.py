...
import mcpython.server.command.old.CommandEntry
import mcpython.server.command.old.CommandParser
import mcpython.server.command.old.Selector
...
def register_command(registry, command):
    if issubclass(
        command, mcpython.server.command.old.Command.Command
    ):  # is it an command
        G.commandparser.add_command(command)
    elif issubclass(
        command, mcpython.server.command.old.CommandEntry.CommandEntry
    ):  # or an command entry
        commandregistry.command_entries[command.NAME] = command
    elif issubclass(
        command, mcpython.server.command.old.Selector.Selector
    ):  # or an selector?
...
mcpython.common.mod.ModMcpython.mcpython.eventbus.subscribe(
    "stage:command:entries", mcpython.server.command.old.CommandEntry.load
)
mcpython.common.mod.ModMcpython.mcpython.eventbus.subscribe(
    "stage:command:selectors", mcpython.server.command.old.Selector.load
)
