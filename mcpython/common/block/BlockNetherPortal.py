...
class NetherPortalBlock(mcpython.common.block.Block.Block):
    ...
    def get_model_state(self) -> dict: return {"axis": self.axis}

    def set_model_state(self, state: dict): self.axis = state.setdefault("axis", "x")
    ...
    def check_valid_surrounding(self):
        ...
        if self.check_valid_block((x, y + 1, z), chunk): return
        if self.check_valid_block((x, y - 1, z), chunk): return
        if self.axis == "x":
            if self.check_valid_block((x + 1, y, z)): return
            if self.check_valid_block((x - 1, y, z)): return
        elif self.axis == "z":
            if self.check_valid_block((x, y, z + 1)): return
            if self.check_valid_block((x, y, z - 1)): return

    def check_valid_block(self, position: tuple, chunk=None):
        if chunk is None: chunk = G.world.get_active_dimension().get_chunk_for_position(position)
        block = chunk.get_block(position)
        if block is None or (block.NAME if type(block) != str else block) not in self.PORTAL_HOLDING_BLOCKS:
    ...
    def on_no_collide_collide(self, player, previous: bool):
        ...
        if time.time() - player.in_nether_portal_since >= 4 or player.gamemode in (1, 3):
            logger.println("[PORTAL] changing dimension nether <-> overworld... (from: '{}')".format(player.dimension.name))
