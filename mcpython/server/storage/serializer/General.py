...
class General(mcpython.server.storage.serializer.IDataSerializer.IDataSerializer):
    ...
    def apply_part_fixer(cls, savefile, fixer):
        ...
        if savefile.version != mcpython.server.storage.SaveFile.LATEST_VERSION: return
    ...
    def load(cls, savefile):
        data = savefile.access_file_json("level.json")
        if data is None: raise mcpython.server.storage.serializer.IDataSerializer.MissingSaveException("level.json not found!")
