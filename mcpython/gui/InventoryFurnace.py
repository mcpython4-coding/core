...
class InventoryFurnace(mcpython.gui.Inventory.Inventory):
    ...
    def create_slots(self) -> list:
        ...
        slots = [mcpython.gui.Slot.Slot(on_update=self.on_input_update, on_shift_click=self.on_shift),
                 mcpython.gui.Slot.Slot(on_update=self.on_fuel_slot_update, on_shift_click=self.on_shift,
                               allowed_item_test=self.is_fuel),
                 mcpython.gui.Slot.Slot(on_update=self.on_output_update, on_shift_click=self.on_shift)]
        return slots
    ...
    def on_activate(self):
        ...
        try:
            ...
            mcpython.event.EventHandler.PUBLIC_EVENT_BUS.unsubscribe("gameloop:tick:end", self.on_tick)
        except ValueError:
            pass
        mcpython.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("user:keyboard:press", self.on_key_press)
        mcpython.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("gameloop:tick:end", self.on_tick)
    ...
    def finish(self):
        if self.slots[2].itemstack.is_empty():
            self.slots[2].set_itemstack(mcpython.gui.ItemStack.ItemStack(self.recipe.output))
        else:
