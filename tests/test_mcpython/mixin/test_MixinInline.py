...
from mcpython.mixin.MixinProcessors import MethodInlineProcessor
...
class TestMixinInline(TestCase):
    def setUp(self):
        global TEST_TARGET
        TEST_TARGET = 0
    ...
    def test_parameter_redirect_twice_inline_one(self):
        ...
        helper = MixinPatchHelper(a)
        processor = MethodInlineProcessor("b", target_accessor=lambda: b, matcher=CounterMatcher(1))
        processor.apply(None, helper.patcher, helper)
        helper.store()
        helper.patcher.applyPatches()

        # If we inline correctly, this should remain in the old state
        self.assertEqual(helper.instruction_listing[14].opcode, PyOpcodes.CALL_FUNCTION)
