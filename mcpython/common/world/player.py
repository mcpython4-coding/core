...
class Player(mcpython.common.entity.Entity.Entity):
    RENDERER = mcpython.client.rendering.EntityRenderer.EntityRenderer("minecraft:player")
    ...
    def __init__(self, name="unknown", dimension=None):
        ...
        if not globals.modloader.finished:
            mcpython.common.mod.ModMcpython.mcpython.eventbus.subscribe("stage:inventories", self.create_inventories,
                                                                 info="setting up player inventory")
        else:
            self.create_inventories()

        mcpython.common.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("hotkey:get_player_position", self.hotkey_get_position)
        mcpython.common.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("hotkey:gamemode_1-3_toggle", self.toggle_gamemode)
    ...
    def create_inventories(self):
        ...
        self.inventories['main'] = mcpython.client.gui.InventoryPlayerMain.InventoryPlayerMain(hotbar)
        self.inventories['chat'] = mcpython.client.chat.Chat.ChatInventory()
        self.inventories["enderchest"] = mcpython.client.gui.InventoryChest.InventoryChest()
        ...
        if mcpython.ResourceLocator.exists("build/texture/gui/icons/xp_bar_empty.png"):
            ...
            mcpython.common.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("stage:blockitemfactory:finish", self.load_xp_icons)

    def load_xp_icons(self):
        self.iconparts = [(mcpython.ResourceLocator.read("build/texture/gui/icons/xp_bar_empty.png", "pyglet"),
                           mcpython.ResourceLocator.read("build/texture/gui/icons/xp_bar.png", "pyglet"))]
    ...
    def pick_up(self, itemstack: mcpython.client.gui.ItemStack.ItemStack) -> bool:
        ...
        mcpython.client.gui.Slot.Slot, mcpython.client.gui.Slot.SlotCopy): itemstack = itemstack.get_itemstack()
    ...
    def kill(self, drop_items=True, kill_animation=True, damage_source: mcpython.common.entity.DamageSource.DamageSource = None, test_totem=True):
