...
class Model:
        ...
        if "elements" in data:
            ...
            for element in data["elements"]:
                self.boxmodels.append(
                    mcpython.client.rendering.BoxModel.BoxModel(element, self)
                )

    def add_face_to_batch(self, position, batch, config, face):
        if not self.drawable:
            raise NotImplementedError(
                "can't draw an model which has not definined textures"
            )
        data = []
    ...
    def get_texture_position(self, name: str) -> tuple:
        ...
        if name in self.texturerename:
            return self.get_texture_position(self.texturerename[name])
        if name.startswith("#"):
            fname = name[1:]
            if fname in self.texture_addresses:
                return self.texture_addresses[fname]
            if fname in self.texturerename:
                return self.get_texture_position(self.texturerename[fname])
