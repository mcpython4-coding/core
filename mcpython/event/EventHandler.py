...
class EventHandler:
    ...
    def create_bus(self, *args, active=True, **kwargs) -> mcpython.event.EventBus.EventBus:
        bus = mcpython.event.EventBus.EventBus(*args, **kwargs)
        self.buses.append(bus)
        if active: self.active_buses.append(bus)
        return bus

    def activate_bus(self, bus: mcpython.event.EventBus.EventBus):
        if bus in self.active_buses: return
        self.active_buses.append(bus)

    def deactivate_bus(self, bus: mcpython.event.EventBus.EventBus):
