...
class BlockFaceState:
    ...
    def show_face(self, face: mcpython.util.enums.EnumSide):
        ...
        if self.custom_renderer is not None:
            if issubclass(type(self.custom_renderer), mcpython.client.rendering.ICustomBlockRenderer.ICustomBatchBlockRenderer):
                self.face_data[face] = self.custom_renderer.add(self.block.position, self.block, face)
            elif issubclass(type(self.custom_renderer), mcpython.client.rendering.ICustomBlockRenderer.ICustomDrawMethodRenderer):
                if not self.subscribed_renderer:
                    mcpython.common.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("render:draw:3d", self._draw_custom_render)
        ...
        if self.custom_renderer is not None:
            if issubclass(type(self.custom_renderer), mcpython.client.rendering.ICustomBlockRenderer.ICustomBatchBlockRenderer):
                ...
                if self.subscribed_renderer and not any(self.faces.values()):
                    mcpython.common.event.EventHandler.PUBLIC_EVENT_BUS.unsubscribe("render:draw:3d", self._draw_custom_render)
    ...
    def hide_all(self):
        ...
        if any(self.faces.values()) and issubclass(
                type(self.custom_renderer), mcpython.client.rendering.ICustomBlockRenderer.ICustomDrawMethodRenderer) and \
