...
class Mod:
        ...
        if type(version) != tuple:
            if type(version) == str:
                ...
                raise ValueError("can't use mod named '{}' as it provides an not-valid version '{}'".format(
                    name, version))
        self.name = name
        self.eventbus: mcpython.event.EventBus.EventBus = mcpython.common.event.EventHandler.LOADING_EVENT_BUS.create_sub_bus(
    ...
    def add_load_default_resources(self, path_name=None):
        ...
        self.eventbus.subscribe("stage:mod:init",
                                lambda: mcpython.ResourceLocator.add_resources_by_modname(self.name, path_name),
