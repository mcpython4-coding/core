...
class TagGroup:
    ...
    def add_from_data(self, tagname: str, data: dict, replace=True):
        if replace:
            self.tags.setdefault(tagname, mcpython.tags.Tag.Tag.from_data(self, tagname, data))
        else:
            self.tags.setdefault(tagname, mcpython.tags.Tag.Tag(self, tagname, [])).entries.extend(data["values"])
