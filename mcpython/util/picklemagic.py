    ...
    *name* is the name of the module and should be a ``"."`` separated
    ...
    def __setattr__(self, name, value):
        ...
        if (
            ...
            and isinstance(self.__dict__[name], FakeModule)
...
class SafeUnpickler(FakeUnpickler):
    ...
    *encoding* and *errors*. *class_factory* can be used to control how the missing class
    ...
    def __init__(
        ...
        FakeUnpickler.__init__(
            self, file, class_factory, encoding=encoding, errors=errors
        )
...
def loads(string, class_factory=None, encoding="bytes", errors="errors"):
    ...
    """
    return FakeUnpickler(
        StringIO(string), class_factory, encoding=encoding, errors=errors
    ).load()
...
def safe_load(
    ...
    *encoding* and *errors*. *class_factory* can be used to control how the missing class
    ...
    return SafeUnpickler(
        file, class_factory, safe_modules, use_copyreg, encoding=encoding, errors=errors
    ).load()
...
def remove_fake_package(name):
    ...
    loaders = [
        i for i in sys.meta_path if isinstance(i, FakePackageLoader) and i.root == name
    ]
