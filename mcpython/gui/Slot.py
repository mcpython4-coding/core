...
class Slot:
    ...
    def __init__(self, itemstack=None, position=(0, 0), allow_player_remove=True, allow_player_insert=True,
        ...
        if self.__itemstack.item:
            pos, index = mcpython.item.ItemHandler.items.itemindextable[self.__itemstack.get_item_name()][
    ...
    def set_itemstack(self, stack, update=True, player=False):
        self.__itemstack = stack if stack is not None else mcpython.gui.ItemStack.ItemStack.get_empty()
        if update:
    ...
    def load(self, data):
        self.set_itemstack(mcpython.gui.ItemStack.ItemStack(data["itemstack"]["itemname"], data["itemstack"]["amount"]))
        if not self.itemstack.is_empty():
...
class SlotCopy:
    def __init__(self, master, position=(0, 0), allow_player_remove=True, allow_player_insert=True,
        ...
        if self.get_itemstack().item:
            pos, index = mcpython.item.ItemHandler.items.itemindextable[self.get_itemstack().get_item_name()][
...
class SlotInfiniteStackExchangeable(Slot):
    ...
    def set_itemstack(self, stack, update=True, player=False):
        self.__itemstack = stack if stack else mcpython.gui.ItemStack.ItemStack.get_empty()
