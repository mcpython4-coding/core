...
class LootTableHandler:
    ...
    def parse_function(self, data: dict) -> mcpython.loot.LootTableFunction.ILootTableFunction:
        name = data["function"]
...
class LootTablePoolEntry:
    ...
    def roll(self, *args, **kwargs):
        ...
        if self.entry_type == LootTablePoolEntryType.ITEM:
            items.append(mcpython.gui.ItemStack.ItemStack(self.name))
        elif self.entry_type == LootTablePoolEntryType.TAG:
            if self.expand:
                items.append(mcpython.gui.ItemStack.ItemStack(random.choice(G.taghandler.get_tag_for(self.name, "items"))))
            else:
                items += [mcpython.gui.ItemStack.ItemStack(name) for name in G.taghandler.get_tag_for(self.name, "items")]
...
mcpython.mod.ModMcpython.mcpython.eventbus.subscribe("stage:loottables:locate", init)
