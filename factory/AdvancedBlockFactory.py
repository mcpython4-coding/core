...
import factory.BlockModelFactory
import event.Registry
import globals as G


class IAdvancedBlockFactoryMode(event.Registry.IRegistryContent):
...
advanced_block_factory_mode_registry = event.Registry.Registry("advanced_block_factory_mode",
...
class FullCube(IAdvancedBlockFactoryMode):
    ...
    def work(cls, factory_instance, settings: dict):
        if "texture" in settings:
            ...
            factory.BlockModelFactory.BlockModelFactory().setName(factory_instance.name).setParent("block/cube_all").\
