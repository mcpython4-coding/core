...
PYGLET_IMAGE_HOVERING = pyglet.sprite.Sprite(
    mcpython.ResourceLocator.read("assets/minecraft/textures/gui/hotbar_selected.png", "pyglet"))
...
class Slot:
    ...
    def __init__(self, itemstack=None, position=(0, 0), allow_player_remove=True, allow_player_insert=True,
        ...
        """
        self.__itemstack = itemstack if itemstack else mcpython.client.gui.ItemStack.ItemStack.get_empty()
        self.position = position
        if self.__itemstack.item:
            pos, index = mcpython.common.item.ItemHandler.items.itemindextable[self.__itemstack.get_item_name()][
                self.__itemstack.item.get_active_image_location()]
            image = mcpython.common.item.ItemHandler.TEXTURE_ATLASES[index].group[tuple(pos)]
    ...
    def set_itemstack(self, stack, update=True, player=False):
        self.__itemstack = stack if stack is not None else mcpython.client.gui.ItemStack.ItemStack.get_empty()
        if update:
    ...
    def draw(self, dx=0, dy=0, hovering=False):
        ...
        if not self.itemstack.is_empty() and (self.itemstack.item.get_default_item_image_location() !=
            ...
            image = mcpython.common.item.ItemHandler.items.itemindextable[self.itemstack.get_item_name()][
        ...
        if self.sprite is not None:
            ...
            # self.sprite.draw()
            mcpython.client.rendering.model.ItemModel.handler.draw(self.itemstack.get_item_name(), (self.position[0] + dx, self.position[1] + dy), "gui", {})
    ...
    def load(self, data):
        self.set_itemstack(
            mcpython.client.gui.ItemStack.ItemStack(data["itemstack"]["itemname"], data["itemstack"]["amount"]))
        if not self.itemstack.is_empty():
...
class SlotCopy:
    def __init__(self, master, position=(0, 0), allow_player_remove=True, allow_player_insert=True,
        ...
        if self.get_itemstack().item:
            pos, index = mcpython.common.item.ItemHandler.items.itemindextable[self.get_itemstack().get_item_name()][
...
class SlotInfiniteStackExchangeable(Slot):
    ...
    def set_itemstack(self, stack, update=True, player=False):
        self.__itemstack = stack if stack else mcpython.client.gui.ItemStack.ItemStack.get_empty()
