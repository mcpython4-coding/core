...
class IStateConfigEntry(mcpython.event.Registry.IRegistryContent):
    """
...
entry_registry = mcpython.event.Registry.Registry("state_definition_entries", ["minecraft:state_definition_entry"])
...
class UIButtonDefaultStateConfigEntry(IStateConfigEntry):
    ...
    def deserialize(cls, state_instance, data: dict,
        ...
        if existing is not None and issubclass(type(existing), mcpython.state.ui.UIPartButton.UIPartButton):
            on_press = getattr(state_instance, data["on_press"]) if "on_press" in data else None
...
class UILableStateConfigEntry(IStateConfigEntry):
    ...
    def deserialize(cls, state_instance, data: dict, existing: typing.Union[
        ...
        if existing is not None and issubclass(type(existing), mcpython.state.ui.UIPartLable.UIPartLable):
            existing.text = text
            existing.position = position
            existing.anchor_window = anchor_window
...
class StateConfigFile:
    ...
    def __init__(self, file: str):
        ...
        mcpython.event.EventHandler.PUBLIC_EVENT_BUS.subscribe("command:reload:end", self.reload)

    def inject(self, state_instance: typing.Union[mcpython.state.State.State, mcpython.state.StatePart.StatePart]):
        """
    ...
    def __del__(self):
        if mcpython is not None and mcpython.event.EventHandler.PUBLIC_EVENT_BUS is not None:
            mcpython.event.EventHandler.PUBLIC_EVENT_BUS.unsubscribe("command:reload:end", self.reload)
