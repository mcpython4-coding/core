...
@G.registry
class CommandReplaceItem(mcpython.chat.command.Command.Command):
    """
    ...
    def parse(cls, values: list, modes: list, info):
        if values[0] == "block":
            ...
            if block is not None and type(block) != str:
                ...
                slot.set_itemstack(mcpython.gui.ItemStack.ItemStack(values[3], 1 if len(values) == 4 else values[4]))
            else:
